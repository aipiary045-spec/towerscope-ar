 ...
 * screen position using camera field-of-view. Not world-locked AR.
 */
class CompassSightOverlayView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        listOf(-30.0, -15.0, 15.0, 30.0).forEach { bearing ->
            val x = bearingToScreenX(bearing, centerX, w) ?: return@forEach
            canvas.drawLine(x, h * 0.14f, x, h * 0.20f, tickPaint)
        }

        markers.forEach { marker ->
            val x = bearingToScreenX(marker.relativeBearingDegrees, centerX, w) ?: return@forEach
