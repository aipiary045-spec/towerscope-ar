...
/**
 * Scan nearby APs and recommend the cleanest Wi‑Fi channels per band.
 */
class ChannelPlannerActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<MaterialButton>(R.id.channelPlannerShareButton).setOnClickListener { share() }
        findViewById<MaterialButton>(R.id.channelPlannerBackButton).setOnClickListener {
            scanJob?.cancel()
            finish()
        }
        ensureScan()
    ...
    private fun scan() {
        scanJob?.cancel()
        statusView.text = "Scanning nearby APs…"
        scanJob = lifecycleScope.launch {
            ...
            statusView.text = best?.let { "Best: ch ${it.channel} · ${it.band}" }
                ?: "No APs found — try again"
        }
    }
    ...
    private fun share() {
        ...
        val intent = Intent(Intent.ACTION_SEND).apply {
            type = "text/plain"
            putExtra(Intent.EXTRA_SUBJECT, "WispEaze channel recommendations")
            putExtra(Intent.EXTRA_TEXT, lastReport)
        }
        startActivity(Intent.createChooser(intent, "Share channel plan"))
    }
}
